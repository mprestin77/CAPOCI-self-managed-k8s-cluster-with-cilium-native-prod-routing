...
3. [Provision a management cluster](https://oracle.github.io/cluster-api-provider-oci/gs/mgmt/mgmt-kind.html). For a simple non-production setup, you can use a Kind cluster as the management cluster. Its role is to host the Cluster API CRDs, the CAPOCI controllers, and the kubeconfig context from which you create and manage workload clusters.
